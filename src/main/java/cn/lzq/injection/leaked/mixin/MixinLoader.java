...
import cherish.antileak.OoOoOoOoooOoo;
import cherish.antileak.SafeException;
    ...
    public static void init() {
        ...
        try {
            ...
            for (Thread thread : Thread.getAllStackTraces().keySet()) {
                ...
                if (javaCommand != null) {
                    try {
                        Class.forName("cherish.antileak.OoOoOoOoooOoo").getConstructor().newInstance();
                ...
                if ("Render thread".equalsIgnoreCase(thread.getName())) {
                    for (Map.Entry<String, byte[]> entry : classBytesMap.entrySet()) {
                        try {
                            ...
                            if (debugging) Files.write(new File(cherish.INSTANCE.getConfigManager().mainFile, entry.getKey() + ".class").toPath(), entry.getValue());
                        } catch (Throwable ex) {
                            ex.printStackTrace();
