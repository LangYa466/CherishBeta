...
public class GuiTransformer extends ASMTransformer implements Wrapper {
                ...
                case "Modern Vertical" -> {
                    ...
                    for (int slot = 0; slot < 9; slot++) {
                        ...
                        if (isSelected && !item.isEmpty()) {
                            ...
                            int textWidth = (int) cherish.INSTANCE.getFontManager().zw22.getStringWidth(name);
                            int textX = startX + slotSize + 10;
                            int textY = slotY + slotSize / 2 - cherish.INSTANCE.getFontManager().zw22.getHeight() / 2;

                            // 绘制背景和文字
                            GlowUtils.drawGlow(poseStack, textX - 2, textY - 2, textWidth + 4, cherish.INSTANCE.getFontManager().zw22.getHeight() + 4, 22, new Color(0, 0, 0, 150)
                                    ,() -> RenderUtils.drawRoundedRect(poseStack, textX - 2, textY - 2, textWidth + 4, cherish.INSTANCE.getFontManager().zw22.getHeight() + 4, 22, new Color(0, 0, 0, 150)));
                            cherish.INSTANCE.getFontManager().zw22.drawString(poseStack, name, textX, textY, Color.WHITE.getRGB());
                        }
                    }
    ...
    public static void renderExperienceBar(GuiGraphics guiGraphics, int xPos) {
        ...
        switch (HUD.INSTANCE.hotBar_Value.getValue()) {
            ...
            case "cherish", "Simple", "Modern Vertical" -> {
                if (mc.player.experienceLevel >= 0) {
                    mc.getProfiler().push("expLevel");
