...
public class EntityTransformer extends ASMTransformer implements Wrapper {
    ...
    public static LookEvent onLook(Entity this1, float yaw, float pitch) {
        ...
        return this1 == mc.player ? (LookEvent) cherish.INSTANCE.getEventManager().call(new LookEvent(yaw, pitch)) : new LookEvent(yaw, pitch);
    }

    public static StrafeEvent onStrafe(Entity this1, float slowSize, float yaw, Vec3 motion) {
        if (mc == null || mc.player == null || mc.level == null) return null;

        return this1 == mc.player ? (StrafeEvent) cherish.INSTANCE.getEventManager().call(new StrafeEvent(motion.x, motion.y, motion.z, yaw, slowSize)) : new StrafeEvent(motion.x, motion.y, motion.z, yaw, slowSize);
    }
