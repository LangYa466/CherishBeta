...
public class Insults extends Module {
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        if (attackEntity != null && attackEntity != mc.player && !attackEntity.isAlive()) {
            if (EntityUtils.isAnimal(attackEntity) || EntityUtils.isMob(attackEntity) || (cherish.INSTANCE.getModuleManager().getModule(Teams.class).isEnabled()
                    && ((Teams) cherish.INSTANCE.getModuleManager().getModule(Teams.class)).isSameTeam(attackEntity))) return;
