...
public class Effects extends Module {
    private final ModeSetting mode = new ModeSetting("Mode", this, new String[]{"cherish", "Modern", "Simple"}, "cherish");
    ...
    public void onRender2D(Render2DEvent event) {
        switch (mode.getValue()) {
            case "Modern", "cherish" -> this.drawEffects(event.guiGraphics());
    ...
    private void drawEffects(GuiGraphics guiGraphics) {
        ...
        // Render Effects
        final TrueTypeFont zw18 = cherish.INSTANCE.getFontManager().zw18;
        final TrueTypeFont ten22 = cherish.INSTANCE.getFontManager().tenacityBold22;
        switch (mode.getValue()) {
            case "cherish" -> {
            ...
            case "Modern" -> {
                ...
                if (!effects.isEmpty()) {
                    ...
                    cherish.INSTANCE.getFontManager().icon25.drawString(poseStack, "R", x + 3, y + 6, Color.WHITE.getRGB());
    ...
    private void drawOldEffects(GuiGraphics guiGraphics) {
        ...
        for (MobEffectInstance effectInstance : mc.player.getActiveEffects()) {
            ...
            cherish.INSTANCE.getFontManager().zw18.drawStringWithShadow(poseStack, name + " " + this.intToRoman(level), 30, y + 6, Color.WHITE.getRGB());
            cherish.INSTANCE.getFontManager().zw18.drawStringWithShadow(poseStack, this.formatDuration(duration), 30, y + 18, Color.WHITE.getRGB());
