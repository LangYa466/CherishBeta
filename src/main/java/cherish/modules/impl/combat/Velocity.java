...
import cherish.utils.player.RayCastUtil;
import cherish.utils.wrapper.WrapperUtils;

import java.util.LinkedList;
import java.util.Queue;

import static cherish.modules.impl.combat.KillAura.target;
        ...
        switch (mode_Value.getValue()) {
            ...
            case "Auto Stuck" -> {
                if (target != null
                    ...
                    Stuck stuck = (Stuck) cherish.INSTANCE.getModuleManager().getModule(Stuck.class); stuck.setEnabled(true);
                }

                if (target != null && target != mc.player && RotationUtils.getDistanceToEntity(target) >= range.getValue().floatValue()) {
                    Stuck stuck = (Stuck) cherish.INSTANCE.getModuleManager().getModule(Stuck.class); stuck.setEnabled(false);
                }
            }
                    ...
                    if (attacked_FlightObject.getValue()) {
                        for (Entity entity : mc.level.entitiesForRendering()) {
                            if (entity != null
                                    ...
                                    && !((AntiBot) cherish.INSTANCE.getModuleManager().getModule(AntiBot.class)).isServerBot(entity)
