...
import cherish.utils.item.InventoryUtils;
...
public class KillAura extends Module {
    ...
    public void attack(LivingEntity target) {
        if (mc.player == null
                ...
                || ((Velocity) cherish.INSTANCE.getModuleManager().getModule(Velocity.class)).attacked) return;
        ...
        if (!blocking) {
            switch (autoBlock_Value.getValue()) {
                ...
                case "GoldApple" -> {
                    if (!cherish.INSTANCE.getModuleManager().getModule(NoSlow.class).isEnabled()
