...
import cherish.modules.impl.setting.Theme;
import cherish.utils.font.FontManager;
...
import cherish.utils.ConnectionUtil;
import cherish.utils.misc.EntityUtils;
import cherish.utils.render.ColorUtils;
import cherish.utils.render.GlowUtils;
...
import static cherish.utils.render.RenderUtils.drawImage;
...
public class HUD extends Module {
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        final FontManager fontManager = cherish.INSTANCE.getFontManager();
        ...
        if (rightCustomImageValue.getValue()) {
            ...
            drawImage(event.poseStack(), cherish.INSTANCE.CLIENT_CUSTOM_IMAGE_PNG, x_, y_, imageWidth, imageHeight, 0, 0, imageWidth, imageHeight);
        }
        ...
        switch (waterMark_Value.getValue()) {
            case "Modern Bar" -> {
                ...
                int stringWidth1 = (int) (fontManager.ax20.getStringWidth("cherish") + fontManager.icon33.getStringWidth("Q"));
                ...
                fontManager.ax20.drawString(poseStack, "cherish", x + 22, y + 6, textColor);
            ...
            case "Modern" -> {
                ...
                int stringWidth = (int) cherish.INSTANCE.getFontManager().ax18.getStringWidth(text);
                ...
                fontManager.tenacityBold20.drawString(poseStack, "cherish", x + 23F, y + 4F, -1);
                fontManager.ax20.drawString(poseStack, text, x + 80F, y + 4F, -1);
            }

            case "Simple Borel" -> {
                GlowUtils.drawGlow(poseStack, 10F, 23F, fontManager.borel60.getStringWidth("cherish"), 7F, 22, ColorUtils.applyOpacity(getColor(1), 0.3f));
                fontManager.borel60.drawString(poseStack, "cherish", 10F, 10F, getColor(1).getRGB());
            }

            case "Simple PlymouthRock" -> {
                GlowUtils.drawGlow(poseStack, 10F, 23F, fontManager.PlymouthRock60.getStringWidth("cherish"), 7F, 22, ColorUtils.applyOpacity(getColor(1), 0.3f));
                fontManager.PlymouthRock60.drawString(poseStack, "cherish", 10F, 10F, getColor(1).getRGB());
            }

            case "Simple Extra" -> {
                if (fontValue.is("Minecraft")) {
                    drawImage(poseStack, cherish.INSTANCE.CLIENT_ICON_PNG, 3, 1, 15, 15, 0, 0, 15, 15);
                    WrapperUtils.drawShadow(poseStack, cherish.CLIENT_NAME.substring(0, 1), 20, 4, ColorUtils.rainbow(10, 1).getRGB(), shadow.getValue());
                    WrapperUtils.drawShadow(poseStack, cherish.CLIENT_NAME.substring(1) + " " + cherish.CLIENT_VERSION + " [" + WrapperUtils.getFPS() + "FPS]", 20 + mc.font.width(cherish.CLIENT_NAME.substring(0, 1)), 4, Color.WHITE.getRGB(), shadow.getValue());
                } else {
                    drawImage(poseStack, cherish.INSTANCE.CLIENT_ICON_PNG, 3, 1, 15, 15, 0, 0, 15, 15);
                    fontManager.tenacity20.drawString(poseStack, cherish.CLIENT_NAME.substring(0, 1), 20, 4, ColorUtils.rainbow(10, 1).getRGB(), shadow.getValue());
                    fontManager.tenacity20.drawString(poseStack, cherish.CLIENT_NAME.substring(1) + " " + cherish.CLIENT_VERSION + " [" + WrapperUtils.getFPS() + "FPS]", 20 + fontManager.tenacity20.getStringWidth(cherish.CLIENT_NAME.substring(0, 1)), 4, Color.WHITE.getRGB(), shadow.getValue());
                }
            }

            case "Simple" -> {
                if (fontValue.is("Minecraft")) {
                    WrapperUtils.drawShadow(poseStack, cherish.CLIENT_NAME.substring(0, 1), 5, 4, ColorUtils.rainbow(10, 1).getRGB(), shadow.getValue());
                    WrapperUtils.drawShadow(poseStack, cherish.CLIENT_NAME.substring(1), 5 + mc.font.width(cherish.CLIENT_NAME.substring(0, 1)), 4, Color.WHITE.getRGB(), shadow.getValue());
                } else {
                    fontManager.tenacity20.drawString(poseStack, cherish.CLIENT_NAME.substring(0, 1), 5, 4, ColorUtils.rainbow(10, 1).getRGB(), shadow.getValue());
                    fontManager.tenacity20.drawString(poseStack, cherish.CLIENT_NAME.substring(1), 5 + fontManager.tenacity20.getStringWidth(cherish.CLIENT_NAME.substring(0, 1)), 4, Color.WHITE.getRGB(), shadow.getValue());
                }
            }

            case "LoraSense" -> {
                String text = "§f" + cherish.CLIENT_NAME + "§asense§f" + " - " + mc.player.getName().getString() + " - " + "吉吉岛" + " - " + EntityUtils.getPing(mc.player) + "ms ";
            ...
            case "Classic" -> {
                String text = "      " + " | " + cherish.CLIENT_NAME + " " + cherish.CLIENT_VERSION + " | Name: " + mc.player.getDisplayName().getString() + " | FPS: " + WrapperUtils.getFPS();
                ...
                drawImage(poseStack, cherish.INSTANCE.CLIENT_ICON_PNG, 7, 7, 15, 15, 0, 0, 15, 15);
