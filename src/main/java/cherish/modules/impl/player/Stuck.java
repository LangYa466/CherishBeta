...
import cherish.ui.progressbar.ProgressbarManager;
...
public class Stuck extends Module {
    ...
    public void onUpdate(UpdateEvent event) {
        ...
        if (mc.player.tickCount % 2 == 0 && (KillAura.target == null || !cherish.INSTANCE.getModuleManager().getModule(Scaffold.class).isEnabled())) {
    ...
    public void onRender2D(Render2DEvent event) {
        ...
        if (stuckTick != 0 && !animation.finished(Direction.BACKWARDS)) {
            ...
            switch (HUD.INSTANCE.count_Value.getValue()) {
                case "cherish" ->
                        ProgressbarManager.drawcherishCountInfo(event.poseStack(), "Stuck now...",stuckTick / 20F, y);
                case "Simple" ->
