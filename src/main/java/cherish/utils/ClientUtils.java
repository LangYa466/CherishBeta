...
public class ClientUtils implements Wrapper {
    public static void mc_debugMessage_no_prefix(String debugMessage) {
        if(mc.level == null || mc.player == null || !cherish.INSTANCE.loaded) return;
    ...
    public static void log(String debugMessage) {
        mc_debugMessage_no_prefix("§8[§c§l" + cherish.CLIENT_NAME + "§8]§c§d" + " " + debugMessage);
    }
    ...
    public static void displayIRC(String text) {
        mc_debugMessage_no_prefix("§r§l[§bcherish§r§l] §r" + text);
    }

    public static void throwableBug(String targetName, Throwable e) {
        try {
            // 确保文件路径存在
            File errorFile = new File(cherish.INSTANCE.getConfigManager().mainFile, String.valueOf(System.currentTimeMillis()));
