package cherish.ui.progressbar;
...
import static cherish.utils.render.RenderUtils.drawRoundedRect;
import static cherish.utils.wrapper.Wrapper.mc;
...
public class ProgressbarManager {
    ...
    public static void drawModernCountInfo(PoseStack poseStack, Animation animation, float progress, int y) {
        ...
        // 绘制文字
        cherish.INSTANCE.getFontManager().zw20.drawCenteredString(poseStack, roundedPercentage + "%", x + 71, y + 5F, new Color(225, 225, 225).getRGB());
    ...
    public static void drawSimpleCountInfo(PoseStack poseStack, String text, float progress, float ticks, int y) {
        ...
        // 绘制文字
        cherish.INSTANCE.getFontManager().zw18.drawCenteredString(poseStack, text + ticks + "Tick", x + 45, y + 5, new Color(225, 225, 225, 160).getRGB());
    }

    /**
     * 绘制 cherish(Normal) 的 Progressbar
     * @author Jon_awa
     */
    public static void drawcherishCountInfo(PoseStack poseStack, String text, float progress, int y) {
        ...
        // 绘制文字
        cherish.INSTANCE.getFontManager().ax14.drawString(poseStack, text + "(" + Math.round(target) + "%)", x, y + 5, new Color(225, 225, 225, 160).getRGB());
    }
