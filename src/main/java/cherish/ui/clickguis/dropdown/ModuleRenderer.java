package cherish.ui.clickguis.dropdown;
...
import cherish.ui.clickguis.dropdown.values.impl.BoolValueComponent;
import cherish.ui.clickguis.dropdown.values.impl.ModeValueComponent;
import cherish.ui.clickguis.dropdown.values.impl.NumberValueComponent;
...
public class ModuleRenderer implements Wrapper {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float delta, int x, int y, int width, int height) {
        ...
        HUD hud = (HUD) cherish.INSTANCE.getModuleManager().getModule(HUD.class);
        ...
        if (!components.isEmpty()) {
            cherish.INSTANCE.getFontManager().tenacity20.drawString(
                    guiGraphics.pose(),
                    extended ? "-" : "+",
                    x + width - 10,
                    y + textOffset,
