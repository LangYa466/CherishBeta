...
public class Frame implements Wrapper {
    ...
    public Frame(int x, int y, int width, int height, Category category) {
        ...
        this.categoryIcon = cherish.INSTANCE.getResourcesManager().loadIconFromAbsolutePath(
                cherish.INSTANCE.getResourcesManager().resources.getAbsolutePath() +
                        "\\icon\\category\\" + category.name().toLowerCase() + "_icon.png"
        );
        renderers = new ArrayList<>();

        int offset = height;
        for (Module mod : cherish.INSTANCE.getModuleManager().getModule(category)) {
